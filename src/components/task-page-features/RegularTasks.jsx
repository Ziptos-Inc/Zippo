...
const RegularTasks = ({ regularTasks, completeTask }) => {
    const { user } = useUser();
    const { completedRegularTasks } = useTaskStore();
  ...
  return (
    <div>
      <h2 className='mb-2 font-bold text-[16px]'>Regular Tasks</h2>
      {regularTasks.map((task, index) => (
        ...
        {user?.completedRegularTasks.includes(task.id) || completedRegularTasks.includes(task.id) ? (
